  ...
  apiLogin(account) {
    axios.post("/api/auth/customer/login", account).then((res) => {
      ...
      if (result.success === true) {
        ...
        // 3. Set Context Session
        this.context.setToken(result.token);
        this.context.setCustomer(result.customer);
        this.context.setMycart(mergedCart);

        // 4. Force a master sync to db right now to secure the merged items
        if (localGuestCart.length > 0) {
          axios.put(`/api/customer/cart/${result.customer._id}`, { cart: mergedCart }, {
            headers: { 'x-access-token': result.token }
          }).catch(e => console.error('Failed to sync merged cart on login', e));
        }
