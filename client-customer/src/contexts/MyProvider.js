...
class MyProvider extends Component {
  ...
  // Đồng bộ giỏ hàng sau login/signup (atomic — tránh race condition)
  loginSession = (token, customer, cart = []) => {
    localStorage.setItem('customer_token', token);
    localStorage.setItem('customer_data', JSON.stringify(customer));
    localStorage.setItem('customer_cart', JSON.stringify(cart));

    this.setState({ token, customer, mycart: cart }, () => {
      // Sau khi state đã cập nhật xong, sync cart lên DB
      if (cart.length > 0) {
        axios.put(`/api/customer/cart/${customer._id}`, { cart }, {
          headers: { 'x-access-token': token }
        }).catch(e => console.error('Cart sync after login failed:', e.message));
      }
    });
  };
